...
const EyeIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" stroke="#94A3B8" strokeWidth="1.8" strokeLinecap="round" />
...
export default function LoginPanel({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serverMessage = err.response?.data?.detail || "Invalid email or password.";
  ...
  return (
    <div className="w-[50%] h-screen bg-[#E8ECF4] flex items-center justify-center px-8 flex-shrink-0">
      <div className="w-full max-w-[340px] bg-white rounded-[20px] shadow-[0_4px_32px_rgba(15,23,42,0.10)] px-9 py-10 flex flex-col">
        ...
        <p className="text-center text-[13px] text-[#94A3B8] mb-8 mt-3">Log In to Continue</p>

        {/* Single form wrapping ALL fields so Enter submits and password managers work */}
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[13px] font-semibold tracking-[0.06em] text-[#6B7280]">Password</label>
              <a href="#" className="text-[12.5px] text-[#0D9488] font-semibold no-underline hover:opacity-80 transition-opacity">
                Forgot password?
              </a>
            </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            />
            <label htmlFor="rememberMe" className="text-[13px] text-[#6B7280] cursor-pointer select-none">
              Remember Me
            </label>
          </div>
        ...
        <p className="text-center text-[13px] text-[#64748B]">
          New to cukai.ai?{" "}
          <NavLink to="/getstarted" className="text-[#0F172A] font-semibold underline underline-offset-2 hover:text-[#0D9488] transition-colors">
            Sign Up.
          </NavLink>
        </p>
      </div>
