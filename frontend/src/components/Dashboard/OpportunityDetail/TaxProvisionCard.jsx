...
function TaxProvisionCard({
  ...
  return (
    <section className="h-full rounded-xl border border-border border-r-4 border-r-primary bg-surface p-4 flex flex-col min-h-0">
      {/* Status row */}
      <div className="flex items-center justify-between gap-2 shrink-0">
        <span className="text-xs font-semibold uppercase tracking-wide text-muted">Tax Provision</span>
        {status && (
          <span className="inline-flex items-center gap-1 rounded-full bg-critical-bg px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-critical">
            {status}
          </span>
        )}
      </div>
      ...
      <h2 className="mt-3 font-headings text-sm font-bold text-headings shrink-0">{provision}</h2>
