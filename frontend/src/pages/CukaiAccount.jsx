...
// Cukai Account uses tabs to keep related SME report workflows on one page.
...
// These are mock dashboard values for the Cukai Account prototype.
const summaryCards = [
...
function CukaiAccount() {
  // Controls which Cukai Account tab is displayed.
  ...
  // Resets every search and filter across all Cukai Account tabs.
  ...
  // It keeps navigation inside Cukai Account without creating another route.
    ...
    <main className="min-h-screen bg-background font-body">
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
        <section className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="font-headings text-3xl font-bold tracking-tight text-headings">Cukai Account</h1>
...
// Renders the horizontal Cukai Account tabs.
...
function TabHeader({ title, subtitle }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-[0.05em] text-[#0D9488]">Cukai Account</p>
  ...
  return (
    <ModalShell title={document.name} subtitle="Source document details" onClose={onClose} size="max-w-2xl">
      ...
      <p className="mt-5 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] p-4 text-sm leading-6 text-[#5F5E5A]">
        This source document supports the Cukai Account audit trail and can be included in accountant review packs.
      </p>
    </ModalShell>
  );
}
...
function GenerateReportPanel({ onGenerate }) {
  ...
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-[#5F5E5A]">
        Create a new mock SME report in this account using selected report sections.
      </p>
      <form
...
function UploadReportModal({ onClose, onSave }) {
  ...
  return (
    <ModalShell title="Upload Report" subtitle="Add an uploaded SME business report to the mock account." onClose={onClose}>
      <form
        className="space-y-4"
        onSubmit={(event) => {
          event.preventDefault();
  ...
  return (
    <ModalShell title="Delete report?" subtitle="This only removes the mock report from the frontend list." onClose={onClose}>
      <p className="text-sm leading-6 text-[#5F5E5A]">
        Delete <span className="font-semibold text-[#0F172A]">{report.name}</span> from Cukai Account?
      </p>
