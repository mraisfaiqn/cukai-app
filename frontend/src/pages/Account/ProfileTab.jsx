...
const EntityCard = ({ Icon, name, type, ssmNo, tin, active, actionText }) => (
  <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden text-left">
    <div className="flex justify-between items-start mb-4">
      <div className="flex items-center gap-3">
        <div className="p-2.5 bg-[#f0fdf9] rounded-lg border border-slate-100"><Icon /></div>
        <div>
    ...
    <div className="flex gap-2">
      {active ? (
        <>
          <button className="flex-1 text-center py-2 px-3 text-xs border border-slate-100 rounded-lg font-medium text-[#0F172A] hover:bg-[#f0fdf9] hover:text-[#0D9488] transition-colors duration-150">Edit Details</button>
          <button className="py-2 px-3 text-xs border border-slate-100 rounded-lg font-medium text-[#64748B] hover:bg-slate-50 transition-colors duration-150">Archive</button>
        </>
      ) : (
        <>
          <button className="flex-1 py-2 px-3 text-xs bg-[#0D9488] text-white rounded-lg font-semibold flex items-center justify-center gap-1.5 hover:bg-[#0f766e] transition-colors duration-150"><SwitchIcon />{actionText}</button>
          <button className="py-2 px-3 text-xs border border-slate-100 rounded-lg font-medium text-[#0F172A] hover:bg-[#f0fdf9] hover:text-[#0D9488] transition-colors duration-150">Edit</button>
        </>
      )}
    </div>
  </div>
    ...
    <div className="w-full space-y-8 text-left">
      <div className="flex gap-3.5 rounded-xl border border-slate-100 bg-[#f0fdf9] p-4 text-sm text-[#64748B]">
        ...
        <div className="space-y-1">
          ...
          <p className="leading-relaxed">Switching entities lets you file Form P (Partnership) and Form B (Self-Employed/Sole Prop) from the same account. All data remains siloed per entity for audit compliance.</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button className="inline-flex items-center gap-1.5 rounded-lg bg-[#0D9488] px-3.5 py-2 text-xs font-semibold text-white shadow-sm hover:bg-[#0f766e] transition-colors duration-150"><PlusIcon />Create New Entity</button>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="flex gap-4 rounded-xl border border-slate-100 bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-6 text-white relative overflow-hidden">
          <div className="space-y-2 flex-1">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">Your entities are being synchronized with LHDN e-Filing protocols. Ensure your digital certificate is updated in the "Tax Documents" tab.</p>
          </div>
        ...
        <div className="flex gap-4 rounded-xl border border-slate-100 bg-[#f0fdf9]/30 p-6 relative overflow-hidden">
          <div className="space-y-2 flex-1">
            ...
            <p className="text-xs text-[#64748B] leading-relaxed">Optimizing entity structures for maximum tax relief eligibility seamlessly with standard accounting procedures.</p>
          </div>
